...
void BenchmarkSuite::run_integrated_benchmark(const BenchmarkConfig &config) {
    ...
    for (const auto &[qid, qtext] : topics_) queries.emplace_back(qid, qtext);
    ...
    for(const auto& pair : boolean_results_vec) bool_map[pair.first] = pair.second;
    ...
    cilk_for (size_t i = 0; i < queries.size(); ++i) {
        ...
        for(const auto& sd : reranked_scored) final_results.push_back({sd.id, sd.score});
    ...
    for(const auto& pair : reranked_results_vec) rerank_map[pair.first] = pair.second;
